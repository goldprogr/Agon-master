...
#include "ui_zagncontroldialog.h"
...
std::list<std::string> ZAGNControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAGNControlDialog::listMints;

ZAGNControlDialog::ZAGNControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAGNControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAGNControlDialog::~ZAGNControlDialog()
{
    delete ui;
}

void ZAGNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAGNControlDialog::updateList()
{
...
void ZAGNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAGNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAGN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAGNControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAGNControlDialog::GetSelectedMints()
{
...
void ZAGNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
