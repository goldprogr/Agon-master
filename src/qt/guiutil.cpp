...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Agon address (e.g. %1)").arg("AWSbBnzmNkjDVaYHX7vkL1MqD96pRYWtZo"));
#endif
...
{
    // return if URI is not valid or is no Agon: URI
...
{
    // Convert agon:// to agon:
    //
    //    Cannot handle this later, because agon:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open agon.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Agon.lnk";
}
...
{
    return GetAutostartDir() / "agon.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a agon.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Agon\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the agon app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add agon app to startup item list
