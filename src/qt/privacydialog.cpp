                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zAGN ought to be enough for anybody." - Bill Gates, 2017
    ui->zAGNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzAGNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzAGN->setEnabled(false);
        ui->pushButtonMintzAGN->setToolTip(tr("zAGN is currently disabled due to maintenance."));

        ui->pushButtonSpendzAGN->setEnabled(false);
        ui->pushButtonSpendzAGN->setToolTip(tr("zAGN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zAGNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzAGN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAGN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zAGN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zAGN in ") + 
...
void PrivacyDialog::on_pushButtonSpendzAGN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zAGN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zAGN
        sendzAGN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zAGN
    sendzAGN();
}

void PrivacyDialog::on_pushButtonZAGNControl_clicked()
{
    ZAGNControlDialog* zAGNControl = new ZAGNControlDialog(this);
    zAGNControl->setModel(walletModel);
    zAGNControl->exec();
}

void PrivacyDialog::setZAGNControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzAGNSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzAGN()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Agon Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zAGNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zAGN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " AGN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zAGN</b>";
    ...
    // use mints from zAGN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAGNControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAGNControlDialog::GetSelectedMints();
    }
    ...
    // Clear zagn selector in case it was used
    ZAGNControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zAGN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " AGN, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zAGN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAGN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zAGN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zAGN "));
    ui->labelzAGNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzAGNSyncStatus->setVisible(fShow);
}
